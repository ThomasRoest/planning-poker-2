...
export const SessionPage = () => {
  ...
  if (session === undefined) return <Box width="50%" color={colors.text}>Loading..</Box>;
  if (session === null) return <Text color={colors.text}>Session not found.</Text>;
      ...
      {user && (
        <MainCard borderRadius="3px">
          <Flex justifyContent="space-between" align="center">
            <Heading as="h1" color={colors.heading} mb={3} display="inline" size="2xl">
              {session.title}
            </Heading>
            <Button
              aria-label="Copy url"
          ...
          <Flex alignItems="center" mb={4}>
            <Badge bg={colors.rowSuccessBg} color={colors.rowSuccessIcon} textTransform="uppercase">
              {user.name}
            </Badge>
          </Flex>
          <Button mb={4} onClick={() => resetVotes({ sessionId: session.id })} bg={colors.neutralButtonBg} color={colors.neutralButtonColor}>
            Reset
          </Button>
